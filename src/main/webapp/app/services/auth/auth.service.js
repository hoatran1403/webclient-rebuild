    ...
    function Auth($rootScope, $state, $sessionStorage, $q, Principal, AuthServerProvider, Account, Register, Activate, Password, PasswordResetInit, PasswordResetFinish) {
        ...
        function authorize(force) {
            ...
            function authThen() {
                ...
                if (!isAuthenticated && !($rootScope.toState.name === 'authentication')) {
                    $state.go('authentication');
                }

                // an authenticated user can't access to authentication pages
                if (isAuthenticated && $rootScope.toState.name === 'authentication') {
                ...
                if ($rootScope.toState.data.authorities && $rootScope.toState.data.authorities.length > 0 && !Principal.hasAnyAuthority($rootScope.toState.data.authorities)) {
                    if (isAuthenticated) {
                        ...
                        $state.go('authentication').then(function () {
